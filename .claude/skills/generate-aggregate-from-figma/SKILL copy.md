...
- Functional approach
- Union of types
- Use unions to describe different states of the models. Important. This will protect the business logic.
- Discriminated Unions
- No `any`
- Types and their constraints shall protect the business logic (invariants).
- Reflect different states of a type, especially if the types structure varies. For instance, an alert for a patient shall be raised, only if two blood tests have been registered:
```ts
type AlertMonitor = {
  alert: false,
  testBloods: []
  ...
  alert: false,
  testBloods: [BloodTest]
} | {
  alert: true,
  testBloods: [BloodTest, BloodTest]
}
```
...
- The section `# Flow` describing sticky notes flow as text:
  ...
  - Wrap all the flows/lines in a code snippet.
  ...
  - Don't put Write Models to flows, as they are defined and associated with commands in `# WriteModels`.
  - Invariants, occurring one after another, join with `|`, not `→`.
- If suggested properties have been confirmed, change the comment to `// AI-suggested. Confirmed`.
- The section `History`:
  - Write an entry shortly describing what happened. Include all suggested and applied changes. This is to track a draft from the original source.


# Generate an actual code
- Ask about a FigJam selection link.
- Generate the spec and ask for approval.
- Generate and/or complete existing files with the types, projections, decide and evolve.
- Generate test specs for projections (https://event-driven-io.github.io/emmett/guides/testing.html) and for the decide function.

# Add a version label
- Add a version label, in format `DDMMYYYY-X` (where DD is the date, MM is the month, YYYY is the year and XX is an incremental value, starting from 0), to FigJam selection's title (to the end).
- Add this version label to the history entry.